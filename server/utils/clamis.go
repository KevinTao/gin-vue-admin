...
func GetMemberClaims(c *gin.Context) (*memberReq.CustomClaims, error) {
	token := c.Request.Header.Get("x-token")
	j := NewJWT()
...
func GetMemberID(c *gin.Context) uint {
	if claims, exists := c.Get("claims"); !exists {
		...
		waitUse := claims.(*memberReq.CustomClaims)
		return waitUse.ID
	}
}
...
func GetMemberInfo(c *gin.Context) *memberReq.CustomClaims {
	if claims, exists := c.Get("claims"); !exists {
		if cl, err := GetMemberClaims(c); err != nil {
			return nil
		} else {
